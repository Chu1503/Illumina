import React, { Children, useLayoutEffect, useMemo, useState, useRef, useEffect, useCallback } from 'react';
...
const Carousel = ({ children }) => {
  ...
  // position first element correctly and this will render only once
  useLayoutEffect(() => {
    setTranslateX(containerRef.current.clientWidth * current);
  }, [current]);
  ...
  return (
    <section className={styles.Root}>
      ...
      <button onClick={() => actionHandler('prev')} className={`${styles.Btn} ${styles.BtnLeft}`}>
        {'<'}
      </button>
      <button onClick={() => actionHandler('next')} className={`${styles.Btn} ${styles.BtnRight}`}>
        {'>'}
      </button>
    </section>
  );
